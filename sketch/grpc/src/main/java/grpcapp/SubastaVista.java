...
public class SubastaVista {
    ...
    public SubastaVista() {
        ...
        // Escala 2x el tamaño preferido del área de lista
        listaScroller.setPreferredSize(new Dimension(500, 160));
        ...
        // Escala 2x la ventana y fuentes
        scaleComponentTreeFonts(principal, 2.0f);
        principal.setMinimumSize(new Dimension(800, 800));
        principal.setSize(800, 800);
        principal.setVisible(true);
    }

    // Escala recursivamente las fuentes de todos los componentes
    private void scaleComponentTreeFonts(Component comp, float factor) {
        if (comp == null)
            return;
        if (comp.getFont() != null) {
            comp.setFont(comp.getFont().deriveFont(comp.getFont().getSize2D() * factor));
        }
        if (comp instanceof Container) {
            for (Component child : ((Container) comp).getComponents()) {
                scaleComponentTreeFonts(child, factor);
            }
        }
    }
